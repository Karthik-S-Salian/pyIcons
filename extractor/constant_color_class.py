...
class COLORS:
    ...
    def hsl_to_rgb(h: float, s: float, l: float) -> tuple[float, float, float]:
        ...
        Returns:
        tuple: A tuple containing the RGB values ([0, 255], [0, 255], [0, 255]).
        """
    ...
    def hsv_to_hsl(h: float, s: float, v: float) -> tuple[float, float, float]:
        ...
        Returns:
        tuple: A tuple containing the HSL values ([0, 360], [0, 1], [0, 1]).
        """
        l = (2 - s) * v / 2
        if l != 0:
            if l == 1:
    ...
    def hsl_to_hsv(h: float, s: float, l: float) -> tuple[float, float, float]:
        ...
        Returns:
        tuple: A tuple containing the HSV values ([0, 360], [0, 1], [0, 1]).
        """
        l *= 2.0
        s_prime = s * (l + (1 - abs(2 * l - 1))) / 2.0
        v = (l + s_prime) / 2.0
        if v == 0.0:
            s = 0.0
        else:
    ...
    def rgb_to_hex(r:float, g:float, b:float)->str:
        ...
        Returns:
            str: Hexadecimal color representation (e.g., '#RRGGBB').
        """
    ...
    def hex_to_rgb(hex:str)-> tuple[float, float, float]:
        """
        Convert hexadecimal color representation to RGB color values.

        Args:
            hex_value (str): Hexadecimal color representation (e.g., '#RRGGBB' or #RGB).
        ...
        if len(hex_value) == 3:
            hex_value = "".join(2 * s for s in hex_value)
